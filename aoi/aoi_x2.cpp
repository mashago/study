...
void AOI::move(uint64_t aoi_id, int x, int y)
{
	...
	{
		// check pre obj
		CheckObj *point_obj = aoi_obj->_x_pre;
		while (point_obj)
		{
			if (point_obj->_x <= x)
			{
			...
			CheckObj *next_obj = aoi_obj->_x_next;
	...
	{
		// check next obj
		CheckObj *point_obj = aoi_obj->_x_next;
		while (point_obj)
		{
			if (point_obj->_x >= x)
			{
			...
			CheckObj *next_obj = point_obj->_x_next;
...
std::vector<CheckObj *> AOI::get_around_obj(CheckObj *aoi_obj)
{
	...
	std::vector<CheckObj *> around_vec; 
