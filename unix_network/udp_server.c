...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if (FD_ISSET(fd, &rset)) {
			...
			char buffer[BUFFER_SIZE];
			int size = 0;
			size = recvfrom(fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&client, &len);
			if (size == -1)
			{
			...
			// do send
			sprintf(buffer, "server recv success[%ld]", time(NULL));
			size = sendto(fd, buffer, strlen(buffer), 0, (struct sockaddr *)&client, len);
			if (size < 0) {
				printf("sendto:error %d\n", errno);
				continue;
			}
